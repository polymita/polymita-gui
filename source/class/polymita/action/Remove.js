    ...
    members: {
        ...
        onExecute: function () {
            var properties = this.getManagement().getSettings().properties,
                ...
                msg = polymita.I18n.trans(i18nCatalog, 'Messages', 'CONFIRM-DELETE'),
                ...
                if (response == 'yes') {
                    request.remove(this.getSelectedItem().id, function (response) {
                        if (response.successful) {
                            msg = polymita.I18n.trans(i18nCatalog, 'Messages', 'SUCCESSFUL-DELETE');
                            q.messaging.emit('Application', 'good', msg);
                            this.emitMessaging('execute-remove', { index: this.getSelectedIndex() });
                        } else {
                            msg = polymita.I18n.trans(i18nCatalog, 'Messages', 'FAILED-DELETE');
